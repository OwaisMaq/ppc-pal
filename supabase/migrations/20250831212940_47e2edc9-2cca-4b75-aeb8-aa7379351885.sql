-- Add the FX normalized view for campaign daily data with proper date casting
...
SELECT
  ...
  f.sales_7d_micros,
  f.conv_7d,
  -- Convert to base currency using FX rates (cast timestamp to date)
  (f.cost_micros / 1000000.0) * COALESCE(fx_rate(f.date::date, COALESCE(pc.currency, 'USD'), 'GBP'), 1.0) as spend_gbp,
  (f.sales_7d_micros / 1000000.0) * COALESCE(fx_rate(f.date::date, COALESCE(pc.currency, 'USD'), 'GBP'), 1.0) as sales_gbp,
  -- Also provide USD conversion
  (f.cost_micros / 1000000.0) * COALESCE(fx_rate(f.date::date, COALESCE(pc.currency, 'USD'), 'USD'), 1.0) as spend_usd,
  (f.sales_7d_micros / 1000000.0) * COALESCE(fx_rate(f.date::date, COALESCE(pc.currency, 'USD'), 'USD'), 1.0) as sales_usd,
  -- And EUR conversion
  (f.cost_micros / 1000000.0) * COALESCE(fx_rate(f.date::date, COALESCE(pc.currency, 'USD'), 'EUR'), 1.0) as spend_eur,
  (f.sales_7d_micros / 1000000.0) * COALESCE(fx_rate(f.date::date, COALESCE(pc.currency, 'USD'), 'EUR'), 1.0) as sales_eur,
...
BEGIN
  ...
  SELECT
    ...
    SUM(fx.conv_7d)::bigint as total_conversions,
    CASE 
      WHEN SUM(CASE 
