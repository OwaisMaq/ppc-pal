...
-- Create unique index for negatives snapshot to prevent duplicates
CREATE UNIQUE INDEX IF NOT EXISTS negatives_snapshot_unique_idx 
ON public.negatives_snapshot(profile_id, scope, negative_type, value, COALESCE(campaign_id, ''), COALESCE(ad_group_id, ''), COALESCE(match_type, ''));
