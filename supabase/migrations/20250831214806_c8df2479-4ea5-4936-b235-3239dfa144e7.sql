-- Phase 10: Attribution + Path-to-Purchase + Creative Diagnostics Schema (Fixed)
...
CREATE TABLE IF NOT EXISTS attribution_results (
  ...
  PRIMARY KEY (run_id, level, campaign_id, ad_group_id, target_id)
);
...
CREATE TABLE IF NOT EXISTS creative_performance_daily (
  ...
  profile_id text NOT NULL,
  ad_id text NOT NULL,
  asset_id text DEFAULT '',                       -- nullable if metric is ad-level only
  campaign_id text,
  ad_group_id text,
  clicks bigint DEFAULT 0,
  ...
  PRIMARY KEY (date, profile_id, ad_id, asset_id)
);
...
WHERE r.level='campaign' AND r.campaign_id != ''
GROUP BY 1,2;
