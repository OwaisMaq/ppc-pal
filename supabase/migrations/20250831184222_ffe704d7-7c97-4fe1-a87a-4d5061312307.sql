-- Phase 4A: Amazon Marketing Stream (AMS) schema
...
CREATE TABLE IF NOT EXISTS public.fact_sp_hourly (
  ...
  PRIMARY KEY (hour, profile_id, campaign_id, ad_group_id, COALESCE(target_id,''))
);
...
CREATE TABLE IF NOT EXISTS public.ams_subscriptions_new (
...
CREATE INDEX IF NOT EXISTS idx_ams_subscriptions_new_profile 
ON public.ams_subscriptions_new (profile_id);
