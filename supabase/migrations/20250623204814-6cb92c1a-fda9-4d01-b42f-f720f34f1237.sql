...
-- Resolve Supabase warnings by safely adding missing RLS policies and foreign key constraints

-- Drop existing policies if they exist to avoid conflicts
DROP POLICY IF EXISTS "Users can view their own connections" ON public.amazon_connections;
DROP POLICY IF EXISTS "Users can insert their own connections" ON public.amazon_connections;
DROP POLICY IF EXISTS "Users can update their own connections" ON public.amazon_connections;
DROP POLICY IF EXISTS "Users can delete their own connections" ON public.amazon_connections;
...
DROP POLICY IF EXISTS "Users can view their own ad groups" ON public.ad_groups;
DROP POLICY IF EXISTS "Users can insert their own ad groups" ON public.ad_groups;
DROP POLICY IF EXISTS "Users can update their own ad groups" ON public.ad_groups;
DROP POLICY IF EXISTS "Users can delete their own ad groups" ON public.ad_groups;
...
DROP POLICY IF EXISTS "Users can view their own campaign metrics" ON public.campaign_metrics_history;
DROP POLICY IF EXISTS "Users can insert their own campaign metrics" ON public.campaign_metrics_history;
...
-- Create RLS policies for amazon_connections
...
-- Create RLS policies for ad_groups
...
-- Create RLS policies for campaign_metrics_history
...
-- Add missing foreign key constraints with proper naming (using IF NOT EXISTS where possible)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_campaigns_connection_id') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_ad_groups_campaign_id') THEN
        ALTER TABLE public.ad_groups 
        ADD CONSTRAINT fk_ad_groups_campaign_id 
        FOREIGN KEY (campaign_id) REFERENCES public.campaigns(id) ON DELETE CASCADE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_keywords_adgroup_id') THEN
        ALTER TABLE public.keywords 
        ADD CONSTRAINT fk_keywords_adgroup_id 
        FOREIGN KEY (adgroup_id) REFERENCES public.ad_groups(id) ON DELETE CASCADE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_campaign_metrics_campaign_id') THEN
        ALTER TABLE public.campaign_metrics_history 
        ADD CONSTRAINT fk_campaign_metrics_campaign_id 
        FOREIGN KEY (campaign_id) REFERENCES public.campaigns(id) ON DELETE CASCADE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_optimization_results_connection_id') THEN
        ALTER TABLE public.optimization_results 
        ADD CONSTRAINT fk_optimization_results_connection_id 
        FOREIGN KEY (connection_id) REFERENCES public.amazon_connections(id) ON DELETE CASCADE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_optimization_recommendations_result_id') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_automation_preferences_connection_id') THEN
