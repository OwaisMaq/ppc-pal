-- Phase 5: Dashboard views and indexes for performance data
...
LEFT JOIN (
  ...
  WHERE hour >= current_date - interval '90 days'
  GROUP BY 1,2,3,4
) f ON f.profile_id = c.amazon_campaign_id AND f.campaign_id = c.amazon_campaign_id AND f.date = d.date
WHERE EXISTS (
  SELECT 1 FROM amazon_connections ac WHERE ac.profile_id = c.amazon_campaign_id
)
GROUP BY 1,2,3,4,5;
...
-- Optional materialized view for 30-day window (enable if performance requires it)
-- CREATE MATERIALIZED VIEW IF NOT EXISTS mv_campaign_30d AS
-- SELECT * FROM v_campaign_daily WHERE date >= current_date - 30;